...
func createContainerRequest(customize ...func(*ContainerRequest)) ContainerRequest {
	req := ContainerRequest{
...
func Test_NewReaper(t *testing.T) {
	...
	t.Run("custom-timeouts", func(t *testing.T) {
		testNewReaper(ctx, t,
			...
			createContainerRequest(func(req *ContainerRequest) {
...
func testNewReaper(ctx context.Context, t *testing.T, cfg config.Config, req ContainerRequest) {
	t.Helper()
	...
	require.Equal(t, req.Image, provider.req.Image, "expected image doesn't match the submitted request")
	require.Equal(t, req.ExposedPorts, provider.req.ExposedPorts, "expected exposed ports don't match the submitted request")
	require.Equal(t, req.Labels, provider.req.Labels, "expected labels don't match the submitted request")
	require.Equal(t, req.Mounts, provider.req.Mounts, "expected mounts don't match the submitted request")
	require.Equal(t, req.WaitingFor, provider.req.WaitingFor, "expected waitingFor don't match the submitted request")
	require.Equal(t, req.Env, provider.req.Env, "expected env doesn't match the submitted request")
