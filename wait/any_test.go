...
func TestAnyMultiStrategy_WaitsForAny(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		if s1Done || s2Done {
			t.Fatalf("no waiting should be done: s1=%v, s2=%v", s1Done, s2Done)
		}
		...
		if s2Done {
			t.Fatal("s2 should be not done, but it is")
		}
	})
}
