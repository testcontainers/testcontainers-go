...
func TestContainerStateAfterTermination(t *testing.T) {
	...
	t.Run("nil-state-after-termination/raw-already-set", func(t *testing.T) {
		ctx := context.Background()
		nginx, err := createContainerFn(ctx)
		CleanupContainer(t, nginx)
		require.NoError(t, err)
...
func TestContainerTerminationRemovesDockerImage(t *testing.T) {
	...
	dockerClient, err := NewDockerClientWithOpts(clientCtx)
	require.NoError(t, err)
...
func TestContainerCreationTimesOutWithHttp(t *testing.T) {
	...
	nginxC, err := Run(
		ctx, nginxDelayedImage,
		WithExposedPorts(nginxDefaultPort),
		WithWaitStrategy(wait.ForHTTP("/").WithStartupTimeout(500*time.Millisecond)),
	)
	CleanupContainer(t, nginxC)
...
func TestContainerCreationWaitsForLogAndPortContextTimeout(t *testing.T) {
	...
	c, err := Run(
		...
		WithWaitStrategy(
			wait.ForLog("I love testcontainers-go").WithStartupTimeout(10*time.Second),
...
func TestCMD(t *testing.T) {
	...
	*/
	...
	c, err := Run(ctx, "alpine",
		WithWaitStrategy(wait.ForLog("command override!")),
...
func TestEntrypoint(t *testing.T) {
	...
	*/
	...
	c, err := Run(ctx, "alpine",
		WithWaitStrategy(wait.ForLog("entrypoint override!")),
...
func TestWorkingDir(t *testing.T) {
	...
	*/
	...
	c, err := Run(ctx, "alpine",
		WithEntrypoint("pwd"),
		WithWaitStrategy(wait.ForLog("/var/tmp/test")),
...
func ExampleRun_containerMethods() {
	ctx := context.Background()
	nginxC, err := Run(
		ctx, "nginx:alpine",
...
func ExampleContainer_Start() {
	ctx := context.Background()
	nginxC, err := Run(ctx, "nginx:alpine",
		WithExposedPorts("80/tcp"),
		WithWaitStrategy(wait.ForHTTP("/").WithStartupTimeout(10*time.Second)),
		WithNoStart(),
	)
...
func TestContainerNonExistentImage(t *testing.T) {
	...
	t.Run("context-cancellation-propagated-to-container-creation", func(t *testing.T) {
...
func TestContainerCustomPlatformImage(t *testing.T) {
	...
	t.Run("error/non-existent-platform", func(t *testing.T) {
		t.Parallel()
		nonExistentPlatform := "windows/arm12"
		ctx, cancel := context.WithTimeout(context.Background(), 1*time.Second)
		defer cancel()
		c, err := Run(ctx, "redis:latest", WithImagePlatform(nonExistentPlatform))
		CleanupContainer(t, c)
		require.Error(t, err)
	})

	t.Run("platform-should-be-propagated", func(t *testing.T) {
		t.Parallel()
		ctx := context.Background()
...
func TestContainerRunningCheckingStatusCode(t *testing.T) {
	ctx := context.Background()
	influx, err := Run(
		ctx, "influxdb:1.8.10-alpine",
		WithExposedPorts("8086/tcp"),
		WithWaitStrategy(wait.ForHTTP("/ping").WithPort("8086/tcp").WithStatusCodeMatcher(
...
func TestCustomPrefixTrailingSlashIsProperlyRemovedIfPresent(t *testing.T) {
	...
	ctx := context.Background()
	c, err := Run(ctx, dockerImage,
		WithImageSubstitutors(newPrependHubRegistry(hubPrefixWithTrailingSlash)),
	)
	CleanupContainer(t, c)
	require.NoError(t, err)
