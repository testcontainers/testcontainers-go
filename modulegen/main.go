...
func processTextTemplate(ctx *Context, example Example, tmpl string) error {
	...
	t, err := textTemplate.New(name).Funcs(textFuncMap).ParseFiles(filepath.Join("_template", name))
	if err != nil {
		return err
	}
