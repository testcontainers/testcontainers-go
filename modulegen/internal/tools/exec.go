...
func GoModTidy(cmdDir string) error {
	if err := runCommand(context.Background(), cmdDir, "go", "mod", "tidy"); err != nil {
...
func GoVet(cmdDir string) error {
	if err := runCommand(context.Background(), cmdDir, "go", "vet", "./..."); err != nil {
...
func MakeLint(cmdDir string) error {
	if err := runCommand(context.Background(), cmdDir, "make", "lint"); err != nil {
		return fmt.Errorf("make lint module: %w", err)
	}
	return nil
}

func runCommand(ctx context.Context, cmdDir string, command string, args ...string) error {
