...
func assertExampleDocContent(t *testing.T, example Example, exampleDocFile string) {
	...
	assert.Equal(t, data[2], "## Introduction")
	assert.Equal(t, data[4], "The Testcontainers module for "+title+".")

	assert.Equal(t, data[6], "## Adding this module to your project dependencies")
	assert.Equal(t, data[8], "Please run the following command to add the "+title+" module to your Go dependencies:")
	assert.Equal(t, data[11], "go get github.com/testcontainers/testcontainers-go/"+example.ParentDir()+"/"+lower)
	assert.Equal(t, data[16], "<!--codeinclude-->")
	assert.Equal(t, data[17], "[Creating a "+title+" container](../../"+example.ParentDir()+"/"+lower+"/"+lower+".go)")
	assert.Equal(t, data[18], "<!--/codeinclude-->")
	assert.Equal(t, data[20], "<!--codeinclude-->")
	assert.Equal(t, data[21], "[Test for a "+title+" container](../../"+example.ParentDir()+"/"+lower+"/"+lower+"_test.go)")
	assert.Equal(t, data[22], "<!--/codeinclude-->")
	assert.Equal(t, data[26], "The "+title+" module exposes one entrypoint function to create the "+title+" container, and this function receives two parameters:")
	assert.True(t, strings.HasSuffix(data[29], "(*"+title+"Container, error)"))
	assert.Equal(t, "for "+title+". E.g. `testcontainers.WithImage(\""+example.Image+"\")`.", data[42])
}
