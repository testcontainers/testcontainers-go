...
2. Read the Go context for the **DOCKER_HOST** key. E.g. `ctx.Value("DOCKER_HOST")`. This is used internally for the library to pass the Docker host to the resource reaper.

3. Read the default Docker socket path, without the unix schema. E.g. `/var/run/docker.sock`

4. Read the **docker.host** property in the `~/.testcontainers.properties` file. E.g. `docker.host=tcp://my.docker.host:1234`
...
6. Return an empty string if none of the above are set.
