...
func (c *DockerContainer) Endpoint(ctx context.Context, proto string) (string, error) {
	...
	for port := range inspect.NetworkSettings.Ports {
		if !lowestPort.IsValid() || port.Num() < lowestPort.Num() {
			lowestPort = port
		}
	}
...
func (c *DockerContainer) PortEndpoint(ctx context.Context, port network.Port, proto string) (string, error) {
	host, err := c.Host(ctx)
	if err != nil {
		return "", err
	}
...
func (c *DockerContainer) MappedPort(ctx context.Context, port network.Port) (network.Port, error) {
	inspect, err := c.Inspect(ctx)
	if err != nil {
...
func (c *DockerContainer) Terminate(ctx context.Context, opts ...TerminateOption) error {
	...
	_, removeErr := c.provider.client.ContainerRemove(ctx, c.GetContainerID(), client.ContainerRemoveOptions{
		RemoveVolumes: true,
		Force:         true,
	})
	errs := []error{
...
func (c *DockerContainer) inspectRawContainer(ctx context.Context) (*container.InspectResponse, error) {
	defer c.provider.Close()
	result, err := c.provider.client.ContainerInspect(ctx, c.ID, client.ContainerInspectOptions{})
	if err != nil {
		return nil, err
	}
...
func (c *DockerContainer) ContainerIP(ctx context.Context) (string, error) {
	...
	// Try to find any valid IP from the networks
	for _, v := range networks {
		if v.IPAddress.IsValid() {
			return v.IPAddress.String(), nil
		}
...
func (c *DockerContainer) Exec(ctx context.Context, cmd []string, options ...tcexec.ProcessOption) (int, io.Reader, error) {
	...
	execCfg := processOptions.ExecConfig
	response, err := cli.ExecCreate(ctx, c.ID, client.ExecCreateOptions{
		User:         execCfg.User,
		Privileged:   execCfg.Privileged,
		TTY:          execCfg.Tty,
		AttachStdin:  execCfg.AttachStdin,
		AttachStderr: execCfg.AttachStderr,
		AttachStdout: execCfg.AttachStdout,
		DetachKeys:   execCfg.DetachKeys,
		Env:          execCfg.Env,
		WorkingDir:   execCfg.WorkingDir,
		Cmd:          execCfg.Cmd,
	})
	if err != nil {
...
func (c *DockerContainer) CopyFileFromContainer(ctx context.Context, filePath string) (io.ReadCloser, error) {
	result, err := c.provider.client.CopyFromContainer(ctx, c.ID, client.CopyFromContainerOptions{SourcePath: filePath})
	if err != nil {
		return nil, err
	}
	defer c.provider.Close()
...
func (c *DockerContainer) CopyDirToContainer(ctx context.Context, hostDirPath string, containerParentPath string, fileMode int64) error {
	...
	_, err = c.provider.client.CopyToContainer(ctx, c.ID, client.CopyToContainerOptions{DestinationPath: parent, Content: buff})
	if err != nil {
		return err
	}
	defer c.provider.Close()
...
func (c *DockerContainer) copyToContainer(ctx context.Context, fileContent func(tw io.Writer) error, fileContentSize int64, containerFilePath string, fileMode int64) error {
	...
	_, err = c.provider.client.CopyToContainer(ctx, c.ID, client.CopyToContainerOptions{DestinationPath: "/", Content: buffer})
	if err != nil {
		return err
	}
	defer c.provider.Close()
...
func (p *DockerProvider) CreateContainer(ctx context.Context, req ContainerRequest) (con Container, err error) {
	...
	if req.ShouldBuildImage() {
		...
		if req.AlwaysPullImage {
			...
			imgResult, err := p.client.ImageInspect(ctx, imageName)
			if err != nil {
			...
			if platform != nil && (imgResult.Architecture != platform.Architecture || imgResult.Os != platform.OS) {
				shouldPullImage = true
			}
		}
...
func (p *DockerProvider) findContainerByName(ctx context.Context, name string) (*container.Summary, error) {
	...
	filter := make(client.Filters).Add("name", fmt.Sprintf("^%s$", name))
	result, err := p.client.ContainerList(ctx, client.ContainerListOptions{All: true, Filters: filter})
	if err != nil {
...
func (p *DockerProvider) GetNetwork(ctx context.Context, req NetworkRequest) (network.Inspect, error) {
	result, err := p.client.NetworkInspect(ctx, req.Name, client.NetworkInspectOptions{
		Verbose: true,
	})
	if err != nil {
...
func (p *DockerProvider) ContainerFromType(ctx context.Context, response container.Summary) (ctr *DockerContainer, err error) {
	...
	if health := jsonRaw.State.Health; health != nil {
		ctr.healthStatus = string(health.Status)
	}
...
func (p *DockerProvider) ListImages(ctx context.Context) ([]ImageInfo, error) {
	...
	imageListResult, err := p.client.ImageList(ctx, client.ImageListOptions{})
	if err != nil {
