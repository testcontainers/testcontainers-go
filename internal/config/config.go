...
// maxSessionIDLen is the maximum length of a session ID. The session ID is used to build
// the name of the reaper container ("reaper_" + sessionID), and Docker limits container
// names to 128 characters, so the session ID must leave room for that prefix.
const maxSessionIDLen = 128 - len("reaper_")

var (
	tcConfig     Config
	tcConfigOnce = new(sync.Once)

	// sessionIDRegex matches the session IDs that are safe to use in the resources created
	// by the library. The session ID is used to build the name of the reaper container
	// ("reaper_" + sessionID), and Docker only allows [a-zA-Z0-9][a-zA-Z0-9_.-] there.
	sessionIDRegex = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9_.-]*$`)
)
...
func validateSessionID(sessionID string) error {
	if len(sessionID) > maxSessionIDLen {
		return fmt.Errorf("must be at most %d characters long, got %d", maxSessionIDLen, len(sessionID))
	}

	if !sessionIDRegex.MatchString(sessionID) {
		return errors.New("must start with an alphanumeric character and contain only alphanumeric characters, dots, hyphens and underscores")
	}
