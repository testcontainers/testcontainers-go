...
func Test_LoadImages(t *testing.T) {
	...
	t.Run("Test load image with wrong architecture", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Regexp(t, regexp.MustCompile("content digest .* not found"), err)
	})
...
func Test_LoadImagesWithPlatform(t *testing.T) {
	...
	t.Run("Test load image with wrong architecture", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, fmt.Sprintf("image with reference %s was found but does not provide the specified platform (%s)", img, loadPlatform), err)
	})
