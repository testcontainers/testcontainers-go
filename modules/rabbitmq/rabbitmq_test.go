...
func TestRunContainer_withAllSettings(t *testing.T) {
	...
	rabbitmqContainer, err := rabbitmq.RunContainer(ctx,
		...
		// addVirtualHosts {
		testcontainers.WithStartupCommand(rabbitmq.VirtualHost{Name: "vhost1"}),
		testcontainers.WithStartupCommand(rabbitmq.VirtualHostLimit{VHost: "vhost1", Name: "max-connections", Value: 1}),
		testcontainers.WithStartupCommand(rabbitmq.VirtualHost{Name: "vhost2", Tracing: true}),
		// }
		// addExchanges {
		testcontainers.WithStartupCommand(rabbitmq.Exchange{Name: "direct-exchange", Type: "direct"}),
		testcontainers.WithStartupCommand(rabbitmq.Exchange{
			Name: "topic-exchange",
			Type: "topic",
		}),
		testcontainers.WithStartupCommand(rabbitmq.Exchange{
		...
		// }
		// addQueues {
		testcontainers.WithStartupCommand(rabbitmq.Queue{Name: "queue1"}),
		testcontainers.WithStartupCommand(rabbitmq.Queue{
			Name:       "queue2",
			AutoDelete: true,
			Durable:    false,
		...
		testcontainers.WithStartupCommand(rabbitmq.Queue{VHost: "vhost2", Name: "queue4"}),
		// }
		// addBindings {
		testcontainers.WithStartupCommand(rabbitmq.NewBinding("direct-exchange", "queue1")),
		testcontainers.WithStartupCommand(rabbitmq.NewBindingWithVHost("vhost1", "topic-exchange-2", "queue3")),
		testcontainers.WithStartupCommand(rabbitmq.Binding{
			VHost:           "vhost2",
		...
		// }
		// addUsers {
		testcontainers.WithStartupCommand(rabbitmq.User{
			Name:     "user1",
			Password: "password1",
		}),
		testcontainers.WithStartupCommand(rabbitmq.User{
		...
		// }
		// addPermissions {
		testcontainers.WithStartupCommand(rabbitmq.NewPermission("vhost1", "user1", ".*", ".*", ".*")),
		// }
		// addPolicies {
		testcontainers.WithStartupCommand(rabbitmq.Policy{
		...
		testcontainers.WithStartupCommand(rabbitmq.OperatorPolicy{
