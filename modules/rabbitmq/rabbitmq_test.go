...
func TestRunContainer_withAllSettings(t *testing.T) {
	...
	rabbitmqContainer, err := rabbitmq.RunContainer(ctx,
		...
		// addVirtualHosts {
		testcontainers.WithReadyCommand(VirtualHost{Name: "vhost1"}),
		testcontainers.WithReadyCommand(VirtualHostLimit{VHost: "vhost1", Name: "max-connections", Value: 1}),
		testcontainers.WithReadyCommand(VirtualHost{Name: "vhost2", Tracing: true}),
		// }
		// addExchanges {
		testcontainers.WithReadyCommand(Exchange{Name: "direct-exchange", Type: "direct"}),
		testcontainers.WithReadyCommand(Exchange{
			Name: "topic-exchange",
			Type: "topic",
		}),
		testcontainers.WithReadyCommand(Exchange{
		...
		// }
		// addQueues {
		testcontainers.WithReadyCommand(Queue{Name: "queue1"}),
		testcontainers.WithReadyCommand(Queue{
			Name:       "queue2",
			AutoDelete: true,
			Durable:    false,
		...
		testcontainers.WithReadyCommand(Queue{VHost: "vhost2", Name: "queue4"}),
		// }
		// addBindings {
		testcontainers.WithReadyCommand(NewBinding("direct-exchange", "queue1")),
		testcontainers.WithReadyCommand(NewBindingWithVHost("vhost1", "topic-exchange-2", "queue3")),
		testcontainers.WithReadyCommand(Binding{
			VHost:           "vhost2",
		...
		// }
		// addUsers {
		testcontainers.WithReadyCommand(User{
			Name:     "user1",
			Password: "password1",
		}),
		testcontainers.WithReadyCommand(User{
		...
		// }
		// addPermissions {
		testcontainers.WithReadyCommand(NewPermission("vhost1", "user1", ".*", ".*", ".*")),
		// }
		// addPolicies {
		testcontainers.WithReadyCommand(Policy{
		...
		testcontainers.WithReadyCommand(OperatorPolicy{
