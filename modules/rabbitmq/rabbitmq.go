...
func Run(ctx context.Context, img string, opts ...testcontainers.ContainerCustomizer) (*RabbitMQContainer, error) {
	moduleOpts := []testcontainers.ContainerCustomizer{
		...
		testcontainers.WithExposedPorts(DefaultAMQPPort, DefaultAMQPSPort, DefaultHTTPSPort, DefaultHTTPPort),
	...
	for _, opt := range opts {
		if apply, ok := opt.(Option); ok {
			if err := apply(&settings); err != nil {
				return nil, fmt.Errorf("rabbitmq option: %w", err)
			}
		}
	}
...
func withConfig(b []byte) testcontainers.CustomizeRequestOption {
	return func(req *testcontainers.GenericContainerRequest) error {
		if err := testcontainers.WithEnv(map[string]string{
			"RABBITMQ_CONFIG_FILE": defaultCustomConfPath,
		})(req); err != nil {
			return err
		}
...
func applySSLSettings(sslSettings *SSLSettings) testcontainers.CustomizeRequestOption {
	...
	return func(req *testcontainers.GenericContainerRequest) error {
		...
		if err := testcontainers.WithAdditionalWaitStrategy(wait.ForLog("started TLS (SSL) listener on [::]:5671"))(req); err != nil {
			return err
		}
