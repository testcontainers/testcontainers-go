...
func TestRedpanda(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.Check(t, is.Len(metadata.Brokers, 1))
	...
	assert.Check(t, is.Equal(http.StatusOK, resp.StatusCode))
	...
	assert.Assert(t, is.ErrorContains(results.FirstErr(), ""), kerr.UnknownTopicOrPartition)
}
...
func TestRedpandaWithAuthentication(t *testing.T) {
	...
	resp, err := httpCl.Do(req)
	assert.NilError(t, err)
	assert.Check(t, is.Equal(http.StatusUnauthorized, resp.StatusCode))
	resp.Body.Close()
...
func TestRedpandaProduceWithAutoCreateTopics(t *testing.T) {
	...
	results := kafkaCl.ProduceSync(ctx, &kgo.Record{Topic: "test", Value: []byte("test message")})
	assert.NilError(t, results.FirstErr())
}
...
func TestRedpandaWithTLS(t *testing.T) {
	...
	assert.Assert(t, cert != nil, "failed to generate cert")
	...
	adminAPIURL, err := container.AdminAPIAddress(ctx)
	assert.NilError(t, err)
	assert.Assert(t, strings.HasPrefix(adminAPIURL, "https://"), "AdminAPIAddress should return https url")
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/v1/cluster/health_overview", adminAPIURL), nil)
	assert.NilError(t, err)
	resp, err := httpCl.Do(req)
	assert.NilError(t, err)
	...
	schemaRegistryURL, err := container.SchemaRegistryAddress(ctx)
	assert.NilError(t, err)
	assert.Assert(t, strings.HasPrefix(adminAPIURL, "https://"), "SchemaRegistryAddress should return https url")
	req, err = http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/subjects", schemaRegistryURL), nil)
	assert.NilError(t, err)
	resp, err = httpCl.Do(req)
	assert.NilError(t, err)
...
func TestRedpandaListener_Simple(t *testing.T) {
	...
	assert.Assert(t, is.Contains(string(out), "Message produced by kcat"))
...
func TestRedpandaListener_InvalidPort(t *testing.T) {
	...
	assert.Assert(t, is.Contains(err.Error(), "invalid port on listener redpanda:99092"))
...
func TestRedpandaListener_NoNetwork(t *testing.T) {
	...
	assert.Assert(t, is.Contains(err.Error(), "container must be attached to at least one network"))
}
