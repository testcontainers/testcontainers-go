...
func Run(ctx context.Context, img string, opts ...testcontainers.RequestCustomizer) (*GrafanaLGTMContainer, error) {
	...
	c := &GrafanaLGTMContainer{DockerContainer: ctr}
	...
	c.Printf("Access to the Grafana dashboard: %s", url)

	return c, nil
}

// WithAdminCredentials sets the admin credentials for the Grafana LGTM container
func WithAdminCredentials(user, password string) testcontainers.RequestCustomizer {
