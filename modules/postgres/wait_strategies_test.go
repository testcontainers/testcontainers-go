...
func TestBasicWaitStrategies_reusedContainer(t *testing.T) {
	...
	// recoveries returns how many crash recoveries the container has logged.
	...
	for i := 0; i < 2; i++ {
		// Generate WAL so that the unclean restart below has to run crash
		// recovery, during which postgres does not accept connections yet.
		...
		// Stop with a zero timeout follows the stop signal with an immediate
		// SIGKILL. The session opened above is kept open on purpose: postgres
		// waits for it on SIGTERM, so the SIGKILL always interrupts an unclean
		// shutdown and the next start is guaranteed to run crash recovery.
		noGrace := time.Duration(0)
		require.NoError(t, ctr.Stop(ctx, &noGrace))

		ctr = run()

		// The wait strategy must not unblock before postgres accepts
		// connections: a single attempt with no retries must succeed.
		db = connect(ctr)
		var rows int
		require.NoErrorf(t, db.QueryRowContext(ctx, "SELECT count(*) FROM reuse_wait").Scan(&rows),
			"reuse cycle %d: container reported ready before postgres accepted connections", i)
		require.Equal(t, rowCount, rows)

		// Guard the reproducer itself: every cycle must have gone through
		// crash recovery, otherwise the scenario above degraded silently.
		require.Equalf(t, i+1, recoveries(ctr), "reuse cycle %d: expected the restart to run crash recovery", i)
	}
}
