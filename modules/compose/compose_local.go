...
func (dc *LocalDockerCompose) applyStrategyToRunningContainer() error {
	...
	for k := range dc.WaitStrategyMap {
		...
		f := make(client.Filters).Add("name", containerName).Add("name", composeV2ContainerName).Add("name", k.service)
		containerListOptions := client.ContainerListOptions{Filters: f, All: true}
