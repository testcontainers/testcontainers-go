...
func (r *reaperSpawner) fromContainer(ctx context.Context, sessionID string, provider ReaperProvider, dockerContainer *DockerContainer) (*Reaper, error) {
	logging.Logger.Printf("⏳ Waiting for Reaper %q to be ready", dockerContainer.ID[:8])
	...
	logging.Logger.Printf("🔥 Reaper obtained from Docker for this test session %s", dockerContainer.ID[:8])
...
func (r *Reaper) connect(ctx context.Context) (chan bool, error) {
	...
	go func() {
		...
		if err := r.handshake(conn); err != nil {
			logging.Logger.Printf("Reaper handshake failed: %s", err)
		}
		<-terminationSignal
	}()
