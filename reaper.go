...
var (
	...
	// reaperPort is the port that the reaper listens on.
	reaperPort = nat.Port("8080/tcp")
...
func (r *reaperSpawner) isRunning(ctx context.Context, container Container) error {
...
// reaper returns an existing Reaper instance if it exists and is running. Otherwise, a new Reaper instance
...
func (r *reaperSpawner) fromContainer(ctx context.Context, sessionID string, provider ReaperProvider, container *DockerContainer) (*Reaper, error) {
	Logger.Printf("⏳ Waiting for Reaper port %s to be ready", reaperPort)

	err := wait.ForExposedPortOnly(reaperPort).
		WithPollInterval(100*time.Millisecond).
		WaitUntilReady(ctx, container)
	if err != nil {
		return nil, fmt.Errorf("wait for port %s: %w", reaperPort, err)
	}

	endpoint, err := container.PortEndpoint(ctx, reaperPort, "")
	if err != nil {
		return nil, fmt.Errorf("port endpoint: %w", err)
	}

	Logger.Printf("🔥 Reaper obtained from Docker for this test session %s", container.ID[:8])
...
func (r *reaperSpawner) newReaper(ctx context.Context, sessionID string, provider ReaperProvider) (reaper *Reaper, err error) { //nolint:nonamedreturns // Needed for deferred error check.
	...
	req := ContainerRequest{
		...
		ExposedPorts: []string{string(reaperPort)},
