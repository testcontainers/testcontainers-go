...
func (r *reaperSpawner) lookupContainer(ctx context.Context, sessionID string) (*DockerContainer, error) {
	...
	return backoff.RetryWithData(
		func() (*DockerContainer, error) {
			result, err := dockerClient.ContainerList(ctx, opts)
			if err != nil {
			...
			if len(result.Items) > 1 {
				return nil, fmt.Errorf("found %d reaper containers for session ID %q", len(result.Items), sessionID)
			}

			r, err := provider.ContainerFromType(ctx, result.Items[0])
			if err != nil {
				return nil, fmt.Errorf("from docker: %w", err)
			}

			switch {
			case r.healthStatus == string(container.Healthy),
				r.healthStatus == string(container.NoHealthcheck):
				return r, nil
			case r.healthStatus != "":
