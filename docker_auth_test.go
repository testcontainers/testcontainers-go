...
func removeImageFromLocalCache(t *testing.T, image string) {
	...
	testcontainersClient, err := testcontainers.NewDockerClientWithOpts(ctx, client.WithVersion(daemonMaxVersion))
	if err != nil {
...
func prepareLocalRegistryWithAuth(t *testing.T) {
	...
	registryC, err := testcontainers.GenericContainer(ctx, genContainerReq)
	require.NoError(t, err)
...
func prepareRedisImage(ctx context.Context, req testcontainers.ContainerRequest, t *testing.T) (testcontainers.Container, error) {
	genContainerReq := testcontainers.GenericContainerRequest{
		ProviderType:     providerType,
		ContainerRequest: req,
		Started:          true,
	...
	redisC, err := testcontainers.GenericContainer(ctx, genContainerReq)
